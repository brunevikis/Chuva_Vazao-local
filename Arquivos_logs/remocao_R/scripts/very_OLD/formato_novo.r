...
cord<- function(q, prev = dat, xlsx = config, input.bacias = bacias) {
  caminhocontornos <-  "C:/Sistemas/ChuvaVazao/remocao_R/Contornos/ETA40/"
...
#models<-"./grid/ECMWF"
config <- read_xlsx("C:/Sistemas/ChuvaVazao/remocao_R/Contornos/Configuracao.xlsx", sheet = "Plan1")
  ...
  for (p in prev) {
    ...
    for (d in maplist) {
      ...
      txt<- left_join(coord,a,by = c("lat", "lon")) %>% group_by(`Codigo ANA`, Latitude, Longitude) %>%
        rename(X = paste0("X",gsub("(.*[0-9]a)|(\\.dat)", "", d))) %>% summarise(prec = round(mean(X,na.rm = T), 1),.groups = 'drop') %>% 
    ...
    x<-x %>% pivot_longer(cols = 6:last_col(), names_to = "date", values_to = "prec") %>% 
      mutate(date = as.Date(date, "X%d%m%y")) %>%group_by(`Codigo ANA`, Latitude, Longitude, date) %>% 
