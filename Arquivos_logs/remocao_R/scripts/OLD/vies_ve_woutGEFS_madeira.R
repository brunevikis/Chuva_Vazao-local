...
vies <- function(dia_remocao, dia_previsao, modelos, grade) {
  ...
  planilha <- read_xlsx("P:/Pedro/remoção_R/Contornos/configuracao_madeira.xlsx",sheet = "Plan1")
  ...
  for ( i in 1:nrow(bacias)){
    ...
    precs<-rbind(precs, res[[i]][[2]][,1]*res[[i]][[3]][,1]*precs2[1,] 
                 + ifelse(is.na(res[[i]][[2]][,2]),0,res[[i]][[2]][,2]*res[[i]][[3]][,2])*precs2[2,] 
                 #+ ifelse(is.na(res[[i]][[2]][,3]),0,res[[i]][[2]][,3]*res[[i]][[3]][,3])*precs2[3,]
    )
