...
ifelse(!dir.exists(paste0("./Arq_Saida/", modelo, "-limite")), dir.create(paste0("./Arq_Saida/", modelo, "-limite")), FALSE)
...
for (m in maplist) {
  ...
  paste0( "p", substring(m, unlist(regexec("[0-9]{6}a",m)), unlist(regexec("a",m))),
		 ...
		 cat(n)
  file.create(paste0("./Arq_Saida/", modelo, "-limite/", n))
  for (count.bac in seq(from = 1, to = length(lookup.xlsx$'Latitude'), by = 1)) {
    ...
    write(psat, file = paste0("./Arq_Saida/", modelo, "-limite/", n), append = T)
  }
  cat(" -> ", n, "  OK\n")
}