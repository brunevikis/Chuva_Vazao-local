...
args = c("20/06/24", "27/06/24", "04/07/24", "11/07/24")
library(readxl)
library(tidyverse)
...
if (length(args) != 0) {
  for (g in 1:length(args)) {
    ...
    if (TRUE) {
    #if (dia_remocao>=as.Date("29/06/25", "%d/%m/%y")) {
      #load("./rem2.RData")
	  load("./rem.RData")
      ...
      colnames(w)<-c("fday","Nome","Latitude","Longitude","Codigo ANA","c ECMWF","c GEFS","c ETA40","p ECMWF","p GEFS","p ETA40")
      ...
      d1<-p%>%filter(date>as.Date(dia_remocao,"%Y%m%d"))%>%mutate(ETA40=coalesce(ETA40,ECMWF),GEFS=coalesce(GEFS,ECMWF))%>%arrange(date)%>%group_by(`Codigo ANA`)%>% mutate(fday = row_number())
      p<-left_join(w,d1, by=c("Codigo ANA"="Codigo ANA","Latitude"="Latitude","Longitude"="Longitude","fday"="fday"))%>%
        mutate(prec=ECMWF*`c ECMWF`*`p ECMWF`+GEFS*`c GEFS`*`p GEFS`+ETA40*`c ETA40`*`p ETA40`)
      n<-length(setdiff(unique(p$date),NA))
      precs<-p[,c(2,12,17)]%>%pivot_wider(names_from = Nome,values_from = prec)%>%.[,-1]
      precs2<-precs[1:(n),]
      l<-nrow(precs2)
