...
namespace Report
{
    class PegaDados                                                                                                         //  Localiza os arquivos fontes de dados 
    {
        ...
        public static PegaDados LeituraPrecipitation(string name)
        {
            ...
            if (File.Exists(@"H:\Middle - Preço\Acompanhamento de Precipitação\Previsao_Numerica\" + name))
            {

                String input = System.IO.File.ReadAllText(@"H:\Middle - Preço\Acompanhamento de Precipitação\Previsao_Numerica\" + name);
    ...
    public class Program
    {
        ...
        internal static string caminhoBase = @"P:\Marcos.Albarracin\Relatorio Final\";

        public static string CriarRelatorio2(DateTime data, string caminho = null, bool preliminar = false)
        {
            Tools.addHistory("H:\\TI - Sistemas\\UAT\\ChuvaVazao\\Log\\report.txt", DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss") + "- metodo CriarRelatorio2 - preliminar == " + preliminar);
        ...
        {
            // camPrev = Path.Combine(@"P:\Trading\Acompanhamento Metereologico Semanal\spiderman" , DateTime.Now.ToString("yyyy_MM_dd") , "CONJUNTO00PREV");



            DateTime data1 = data.AddDays(-1);
            //var camNOAA = Path.Combine(@"C:\Compass\MinhaTI\Alex Freires Marques - Compass\Pedro\NOAA", data.ToString("yyyy"), data.ToString("MM"), data.ToString("dd"));
            // var camNOAAOntem = Path.Combine(@"C:\Compass\MinhaTI\Alex Freires Marques - Compass\Pedro\NOAA", data1.ToString("yyyy"), data1.ToString("MM"), data1.ToString("dd"));
            //C:\Compass\MinhaTI\Preço - Documentos\Acompanhamento_de_Precipitacao\Mapas\2021\03\24\GFS00

            var camNOAA = Path.Combine(@"C:\Compass\MinhaTI\Preço - Documentos\Acompanhamento_de_Precipitacao\Mapas", data.ToString("yyyy"), data.ToString("MM"), data.ToString("dd"));
            var camNOAAOntem = Path.Combine(@"C:\Compass\MinhaTI\Preço - Documentos\Acompanhamento_de_Precipitacao\Mapas", data1.ToString("yyyy"), data1.ToString("MM"), data1.ToString("dd"));

            var camAlterNOAA = Path.Combine(@"C:\Compass\OneDrive - MinhaTI\Compass\Pedro\NOAA", data.ToString("yyyy"), data.ToString("MM"), data.ToString("dd"));
            var camAlterNOAAOntem = Path.Combine(@"C:\Compass\OneDrive - MinhaTI\Compass\Pedro\NOAA", data1.ToString("yyyy"), data1.ToString("MM"), data1.ToString("dd"));
            ...
            string caminhoSpider = @"P:\Trading\Acompanhamento Metereologico Semanal\spiderman\";
        ...
        {
            var caminhoMerge = @"H:\Middle - Preço\Acompanhamento de Precipitação\Observado_MERGE";
            var caminhoFunceme = @"H:\Middle - Preço\Acompanhamento de Precipitação\Observado_Funceme";
            var caminhoPrevisao = @"H:\Middle - Preço\Acompanhamento de Precipitação\Previsao_Numerica";
            ...
            Func<DateTime, List<List<string>>> createImgsConj = new Func<DateTime, List<List<string>>>(dataRef =>
            {
                ...
                GradsHelper.Grads.CreateImgFromFiles(imgs0, "Obser + Prev Conj em  " + dataRef.ToString("dd/MM"), semana0.Item1.ToString("dd/MM") + " e " + semana0.Item2.ToString("dd/MM"),
                    ...
                    @"H:\TI - Sistemas\UAT\ChuvaVazao\Auxiliar\CreateImgFromFiles.gs");
            ...
            Func<DateTime, List<List<string>>> createImgsGEFS = new Func<DateTime, List<List<string>>>(dataRef =>
            {
                ...
                for (DateTime dt = semana2.Item1; dt <= semana2.Item2; dt = dt.AddDays(1))
                {
                    ...
                    {
                        fprev = System.IO.Path.Combine(@"H:\Middle - Preço\Acompanhamento de Precipitação\Observado_MERGE\MCP\040", "prec_mct1318_" + dt.ToString("MM") + ".ctl");
            ...
            GradsHelper.Grads.CreateImgDiffFromFiles(dC0[0], dC1[0], "Prev Conj em  " + data.ToString("dd/MM"), semana0.Item1.ToString("dd/MM") + " e " + semana0.Item2.ToString("dd/MM"),
               Path.Combine(caminhoBase, @"imgs_temp", "semana0_conjDiff_" + data.ToString("yyyyMMdd") + ".gif"),
               @"H:\TI - Sistemas\UAT\ChuvaVazao\Auxiliar\CreateImgDiffFromFiles.gs");
